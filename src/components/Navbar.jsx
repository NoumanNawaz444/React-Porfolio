...
export default function Navbar() {
  return (
    <Disclosure
      as="nav"
      className="bg-white dark:bg-gray-800 rounded-md shadow-lg   sm:my-[20px] my-6 border-2 border-black/20 dark:border-white/20"
    >
      {({ open }) => (
        <>
          ...
          <Disclosure.Panel className="md:hidden">
            <div className="space-y-1 px-2 pb-3 pt-2 rounded-md bg-white dark:bg-gray-800 shadow-lg outline-none duration-400 ">
              {navigation.map((item) => (
                <Disclosure.Button className="flex flex-col w-full text-start outline-none duration-300">
                  <NavLink
                        key={item.name}
                        to={item.href}
                        className={({ isActive }) =>
